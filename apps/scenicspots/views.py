...
class ScenicListView(View):
    ...
    def get(self,request):
        ...
        if list_type == 'scenic':
            ...
            result = json.dumps({"status":"failed","msg":"数据来源错误"},ensure_ascii=False)
            return HttpResponse(result)

        return render(request,'scenic_list.html',{'all_spots': all_spots, 'culture': public_box.get('culture'),
            'specialty': public_box.get('specialty'), 'food': public_box.get('food'), 'life': public_box.get('life'),
            'now_type': 'scenic', 'list_type': list_type,
        })
...
class OrderDetailsView(View):
    ...
    def get(self,request,order_num):
        ...
        return render(request,'order_details2.html',{'order':order})